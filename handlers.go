...
function switchTab(tab){
  ...
  document.getElementById('providersTab').style.display=tab==='providers'?'block':'none';
  if(tab==='providers'&&savedKey)loadUsage();
  if(tab!=='providers'&&usageRefreshTimer){clearInterval(usageRefreshTimer);usageRefreshTimer=null;}
}
...
function refresh(){
  ...
  loadStats();
  if(currentTab==='providers')loadUsage();
}
...
function loadUsage(){
  ...
  fetchUsage();
  usageRefreshTimer=setInterval(fetchUsage,30000);
}

function fetchUsage(){
  var key=savedKey;
  document.getElementById('providersPlaceholder').style.display='none';
  document.getElementById('providerGrid').style.display='grid';
  document.getElementById('providersError').style.display='none';
  renderSkeletons();
  fetch('/admin/usage',{headers:{'Authorization':'Bearer '+key}})
    .then(function(r){if(!r.ok)throw new Error(r.status===401?'Admin Key 错误':'请求失败('+r.status+')');return r.json()})
    .then(function(d){renderProviders(d)})
    .catch(function(e){document.getElementById('providersError').textContent=e.message;document.getElementById('providersError').style.display='block';});
}
...
function renderSkeletons(){
  ['codex','minimax','zhipu'].forEach(function(name){
    document.getElementById('card-'+name).innerHTML=
      '<div class="card-header"><div class="skeleton" style="width:80px;height:16px"></div></div>'+
      '<div class="metric"><div class="skeleton" style="width:100%;height:8px;margin-bottom:8px"></div><div class="skeleton" style="width:60%;height:8px"></div></div>'+
      '<div class="metric"><div class="skeleton" style="width:100%;height:8px;margin-bottom:8px"></div><div class="skeleton" style="width:60%;height:8px"></div></div>';
  });
}
...
function renderProviders(d){
  var p=d.providers||{};
  renderCodexCard(p.codex);
  renderMiniMaxCard(p.minimax);
  renderZhipuCard(p.zhipu);
}

function renderCodexCard(c){
  var el=document.getElementById('card-codex');
  if(!c){el.innerHTML='<div class="card-header"><div class="status-dot not_configured"></div><h3>Codex (ChatGPT)</h3></div><div class="not-configured-msg">未配置</div>';return;}
  var dotClass=c.status==='ok'?'ok':c.status==='error'?'error':'not_configured';
  if(c.status==='not_configured'){el.innerHTML='<div class="card-header"><div class="status-dot not_configured"></div><h3>Codex (ChatGPT)</h3></div><div class="not-configured-msg">未配置</div>';return;}
  if(c.status==='error'){el.innerHTML='<div class="card-header"><div class="status-dot error"></div><h3>Codex (ChatGPT)</h3></div><div class="error-msg">'+esc(c.error||'查询失败')+'</div>';return;}
  var planBadge=c.plan?'<span class="plan-badge '+(c.plan==='pro'?'pro':'plus')+'">'+esc(c.plan.toUpperCase())+'</span>':'';
  var w5h=c['5h_window']||{};
  var ww=c.weekly_window||{};
  el.innerHTML=
    '<div class="card-header"><div class="status-dot ok"></div><h3>Codex (ChatGPT)</h3>'+planBadge+'</div>'+
    buildMetric('5 小时窗口','已用 '+w5h.used_percent+'%',w5h.used_percent,w5h.reset_seconds)+
    buildMetric('周窗口','已用 '+ww.used_percent+'%',ww.used_percent,ww.reset_seconds);
}

function renderMiniMaxCard(c){
  var el=document.getElementById('card-minimax');
  if(!c){el.innerHTML='<div class="card-header"><div class="status-dot not_configured"></div><h3>MiniMax</h3></div><div class="not-configured-msg">未配置</div>';return;}
  var dotClass=c.status==='ok'?'ok':c.status==='error'?'error':'not_configured';
  if(c.status==='not_configured'){el.innerHTML='<div class="card-header"><div class="status-dot not_configured"></div><h3>MiniMax</h3></div><div class="not-configured-msg">未配置</div>';return;}
  if(c.status==='error'){el.innerHTML='<div class="card-header"><div class="status-dot error"></div><h3>MiniMax</h3></div><div class="error-msg">'+esc(c.error||'查询失败')+'</div>';return;}
  var w=c['5h_window']||{};
  var pct=w.used_percent||0;
  el.innerHTML=
    '<div class="card-header"><div class="status-dot ok"></div><h3>MiniMax</h3>'+(c.model?'<span class="plan-badge plus">'+esc(c.model)+'</span>':'')+'</div>'+
    buildMetric('5 小时窗口',fmt(c.used||0)+' / '+fmt(c.total||0),pct,w.reset_seconds);
}

function renderZhipuCard(c){
  var el=document.getElementById('card-zhipu');
  if(!c){el.innerHTML='<div class="card-header"><div class="status-dot not_configured"></div><h3>智谱 GLM</h3></div><div class="not-configured-msg">未配置</div>';return;}
  var dotClass=c.status==='ok'?'ok':c.status==='error'?'error':'not_configured';
  if(c.status==='not_configured'){el.innerHTML='<div class="card-header"><div class="status-dot not_configured"></div><h3>智谱 GLM</h3></div><div class="not-configured-msg">未配置</div>';return;}
  if(c.status==='error'){el.innerHTML='<div class="card-header"><div class="status-dot error"></div><h3>智谱 GLM</h3></div><div class="error-msg">'+esc(c.error||'查询失败')+'</div>';return;}
  var planBadge=c.plan?'<span class="plan-badge '+(c.plan==='pro'?'pro':'plus')+'">'+esc(c.plan.toUpperCase())+'</span>':'';
  var mcp=c['mcp_monthly']||{};
  var tok=c['tokens_5h']||{};
  var html=
    '<div class="card-header"><div class="status-dot ok"></div><h3>智谱 GLM</h3>'+planBadge+'</div>'+
    buildMetric('MCP 工具 (月)',fmt(mcp.used||0)+' / '+fmt(mcp.total||0)+' (剩余 '+fmt(mcp.remaining||0)+')',mcp.percentage||0,mcp.reset_seconds);
  if(tok.percentage!==undefined){
    html+=buildMetric('模型 Tokens (5h)','已用 '+tok.percentage+'%',tok.percentage||0,tok.reset_seconds);
  }
  el.innerHTML=html;
}

function buildMetric(label,detailText,pct,resetSec){
  var color=pct<50?'green':pct<80?'yellow':'red';
  var resetHtml=resetSec!=null?'<span class="meta">'+fmtTime(resetSec)+'后重置</span>':'';
  return '<div class="metric">'+
    '<div class="metric-label"><span>'+esc(label)+'</span>'+resetHtml+'</div>'+
    '<div class="progress-bar"><div class="progress-fill '+color+'" style="width:'+Math.min(pct,100)+'%"></div></div>'+
    '<div class="metric-detail">'+detailText+'</div></div>';
}
